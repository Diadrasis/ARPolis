...
{
    public class CustomMarkerEngineGUI : MonoBehaviour
    {
        ...
        /// Prefab of UI Marker
        /// </summary>
        public GameObject markerPoiPrefab, markerActivityPrefab;

        public Canvas canvas;
        public OnlineMaps map;
        private OnlineMapsControlBase control;

        public static Vector2[] allMarkersPositions()
        {
            List<Vector2> pos = new List<Vector2>();
            foreach (CustomMarkerGUI m in markers)
            {
                pos.Add(new Vector2((float)m.lng, (float)m.lat));
            }
            return pos.ToArray();
        }
        ...
        /// </summary>
        public static CustomMarkerEngineGUI instance
        {
        ...
        {
            _instance = this;
            markers = new List<CustomMarkerGUI>();
        ...
        {
            foreach (CustomMarkerGUI marker in markers)
            {
                marker.Dispose();
        ...
        public static void RemoveMarker(CustomMarkerGUI marker)
        {
            OnlineMapsUtils.Destroy(marker.gameObject);
            marker.Dispose();
        ...
        public static void RemoveMarkerAt(int index)
        {
            ...
            CustomMarkerGUI marker = markers[index];
            OnlineMapsUtils.Destroy(marker.gameObject);
            marker.Dispose();
        ...
        private void UpdateMarker(CustomMarkerGUI marker, double tlx, double tly, double brx, double bry)
        {
